...
import { convert, getPoolName, getTokenName, KnownTokenMap, STABLE_COINS } from '../utils/utils';
import { useConnectionConfig } from '../utils/connection';
import { cache, getMultipleAccounts, MintParser, ParsedAccountBase, useCachedPool } from '../utils/accounts';
			...
			if (p.pubkeys.feeAccount) {
				...
				if (poolMint && feeAccount && feeAccount.info.mint.toBase58() === p.pubkeys.mint.toBase58()) {
					...
					if (fees !== 0) {
						...
						const apy0 =
							parseFloat(((baseVolume / daysSinceInception) * 0.003 * 356) as any) / baseReserveUSD;
						const apy1 =
							parseFloat(((quoteVolume / daysSinceInception) * 0.003 * 356) as any) / quoteReserveUSD;
...
export const useEnrichedTokens = (pools: PoolInfo[]) => {
	const context = useContext(MarketsContext);
	const { tokenMap } = useConnectionConfig();
	const [enriched, setEnriched] = useState<any[]>([]);
	const subscribeToMarket = context?.subscribeToMarket;
	...
	useEffect(() => {
		...
		const update = () => {
			setEnriched(createEnrichedTokens(pools, marketsByMint, dailyVolume, tokenMap));
		};
...
function createEnrichedTokens(
	...
	const enrichedTokens = pools.reduce<Record<string, any>>((acc, p) => {
		if (p.pubkeys.holdingMints && p.pubkeys.holdingMints.length > 1) {
			const mints = (p.pubkeys.holdingMints || []).map(a => a.toBase58()).sort();
			const indexA = mints[0] === p.pubkeys.holdingMints[0]?.toBase58() ? 0 : 1;
			const indexB = indexA === 0 ? 1 : 0;
			const accountA = cache.getAccount(p.pubkeys.holdingAccounts[indexA]);
			...
			if (acc[mints[0]]) {
				acc[mints[0]].liquidity += convert(accountA, mintA);
				acc[mints[0]].liquidityInUsd += baseReserveUSD;
				acc[mints[0]].volume24h += volume24h;
			} else {
				acc[mints[0]] = {
					mint: mints[0],
					symbol: getTokenName(tokenMap, mints[0]),
					liquidity: 0,
					liquidityInUsd: 0,
					volume24h: 0,
					price: baseMid,
			...
			if (acc[mints[1]]) {
				acc[mints[1]].liquidity += convert(accountB, mintB);
				acc[mints[1]].liquidityInUsd += quoteReserveUSD;
				acc[mints[1]].volume24h += volume24h;
			} else {
				acc[mints[1]] = {
					mint: mints[1],
					symbol: getTokenName(tokenMap, mints[1]),
					liquidity: 0,
					liquidityInUsd: 0,
					volume24h: 0,
					price: quote,
	...
	return Object.values(enrichedTokens);
}
