...
export const useCurrencyLeg = (defaultMint?: string) => {
	...
	return useMemo(
		() => ({
			...
			sufficientBalance: () => account !== undefined && convert(account, mint) >= parseFloat(amount),
		}),
...
export function CurrencyPairProvider({ children = null as any }) {
	...
	useEffect(() => {
		// set history
		const base = tokens.find(t => t.mintAddress === mintAddressA)?.tokenSymbol || mintAddressA;
		const quote = tokens.find(t => t.mintAddress === mintAddressB)?.tokenSymbol || mintAddressB;
