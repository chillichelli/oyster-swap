...
export const removeLiquidity = async (
	...
	notify({
		message: t`Removing Liquidity...`,
		description: t`Please review transactions to approve.`,
		type: 'warning',
	});
  ...
  if (deleteAccount) {
    ...
    cache.deleteAccount(account.pubkey);
		...
		type: 'success',
		action: () => (
			<Button size="small" component="a" href={`https://explorer.solana.com/transaction/${tx}`}>
...
export const swap = async (
	...
	if (!pool || !components[0].account) {
		notify({
			type: 'error',
			message: t`Pool doesn't exist.`,
			description: t`Swap trade cancelled`,
		});
		return;
	}
...
async function _addLiquidityExistingPool(
	...
	notify({
		message: t`Pool Funded. Happy trading.`,
		type: 'success',
		action: () => (
			<Button size="small" component="a" href={`https://explorer.solana.com/transaction/${tx}`}>
				<Trans>explorer</Trans>
	...
	if (account && !isWrappedSol) {
		...
		if (isWrappedSol) {
			cleanupInstructions.push(
				Token.createCloseAccountInstruction(programIds().token, toAccount, payer, payer, [])
			);
		}
	}
...
async function _addLiquidityNewPool(
	...
	if (components.some(c => !c.account)) {
		notify({
			message: t`You need to have balance for all legs in the basket...`,
			description: t`Please review inputs.`,
			type: 'error',
		});
		return;
	}
...
function getWrappedAccount(
	...
	instructions.push(
		Token.createInitAccountInstruction(programIds().token, WRAPPED_SOL_MINT, account.publicKey, payer)
	);
